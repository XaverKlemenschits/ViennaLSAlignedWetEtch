...
constexpr NumericType gridDelta = 0.92; //0.51;//0.27; // 0.51;
...
int main() {
  ...
  {
    hrleCoordType bounds[2 * D] = {-extent, extent, -2 * extent, 2 * extent};
  ...
  double maskAngle = 0.;//PI / 4.;
  {
    ...
    writeSurface(mask, "fullMask.vtp");

    auto maskHole = LevelSetType::New(mask->getGrid());
    hrleCoordType shortAxis = extent * (4. / 5.);
    hrleCoordType longAxis =  extent + shortAxis;
    hrleCoordType minCorner[D] = {-shortAxis, -longAxis, -1.};
    hrleCoordType maxCorner[D] = {shortAxis, longAxis, 3.};
    ...
    // lsMakeGeometry(maskHole, lsSmartPointer<lsBox<NumericType, D>>::New(minCorner, maxCorner)).apply();

    writeSurface(maskHole, "maskHole.vtp");

    lsBooleanOperation(mask, maskHole, lsBooleanOperationEnum::RELATIVE_COMPLEMENT).apply();

    minCorner[0] = -extent * 2./5.;
    minCorner[1] = - 2 * extent + gridDelta;
    minCorner[2] = 0.;
    maxCorner[0] = extent * 2./5.;
    maxCorner[1] = 0.;
    maxCorner[2] = 2.;
    lsMakeGeometry(plane, lsSmartPointer<lsBox<NumericType, D>>::New(minCorner, maxCorner)).apply();
    lsBooleanOperation(mask, plane, lsBooleanOperationEnum::UNION).apply();

    writeSurface(mask, "mask_initial.vtp");
  }

  lsBooleanOperation(substrate, mask, lsBooleanOperationEnum::UNION).apply();

  writeSurface(substrate, "subs_initial.vtp");

  std::vector<double> epiVels = {0, 40};
  auto velocity = lsSmartPointer<WetEtch>::New(epiVels);
  // auto velocity = lsSmartPointer<epitaxy>::New(epiVels);
  ...
  if(maskAngle < 0.01) {
    writeSurface(substrate, "subs_aligned.vtp");
  } else {
    writeSurface(substrate, "subs_misaligned.vtp");
  }

  std::cout << "Extracting volume mesh..." << std::endl;
